...
import { AUTH_KEY, AuthDecoratorPayload, AuthKey, ConditionKey } from '../constants/auth';
...
export class AuthenticationGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    const authTypeValue = this.reflector.getAllAndOverride<AuthDecoratorPayload | undefined>(AUTH_KEY, [
      context.getHandler(),
      context.getClass(),
    ]) ?? { authKey: [AuthKey.None], option: { condition: ConditionKey.And } };

    const guards = authTypeValue.authKey.map((authType) => this.authTypeGuardMap[authType]);
